/*
 * sha3_test.cpp - Moved main function from sha3.c.
 *
...
unittest()
{
    ...
    sha3_Init256<uint64_t>(&c);
    hash = sha3_Finalize<uint64_t>(&c);
    if(memcmp(sha3_256_empty, hash, sizeof(sha3_256_empty)) != 0) {
        printf("SHA3-256() doesn't match known answer\n");

        printf("Received result: \n");
        for(unsigned int i=0; i<sizeof(sha3_256_empty)/8; i++){
        	printf("%lx ", hash[i]);
        }
        printf("\n");
        return 1;
    ...
    sha3_Init256(&c);
    sha3_Update<uint8_t, uint64_t>(&c, buf, sizeof(buf));
    hash = sha3_Finalize(&c);
    ...
    sha3_Init256(&c);
    sha3_Update(&c, (const uint8_t*) (void *)
...
int main(int argc, char *argv[]){
	...
	assert(params.outputlen_bytes % 32 == 0);


	//SHA3-256("") = a7ffc6f8bf1ed76651c14756a061d662f580ff4de43b49fa82d80a4b80f8434a
	//SHA3-256("a") = 80084bf2fba02475726feb2cab2d8215eab14bc6bdd8bfb2c8151257032ecd8b
	int res = main_with_command_line<uint64_t>(params, "sha3_256_DRBG",
			sha3_256_DRBG,
			sha3_256_DRBG,
			sha3_256_DRBG,
			sha3_256_random_tape_len_in_bytes);
